...
def get_EIG(guess: str, answers: list):
    ...
    for fb in feedback:
        probability = len(feedback[fb])/len(answers) #fix to be len(answers)
        pct_words_removed = (len(answers)-len(feedback[fb]))/len(answers) #fix to be len(answers)
...
def get_best_guess(guesses: list, answers: list) -> str:
    ...
    for i, guess in enumerate(guesses):
        guesses[guess] = get_EIG(guess, answers) #Expected information gain
        #if i%100 == 0:
    ...
    best_guess_EIG = guesses[best_guess]
    best_guess_in_answers = ""
    best_guess_in_answers_EIG = 0

    while best_guess_in_answers not in answers:
        best_guess_in_answers = max(guesses, key = guesses.get)
        best_guess_in_answers_EIG = guesses.pop(best_guess_in_answers)

    #print("Best Guess: " + best_guess + " EIG: " + str(best_guess_EIG))
    #print("Best Guess in Answers: " + best_guess_in_answers + " EIG: " + str(best_guess_in_answers_EIG))
    ...
    """
    odds_of_correct_guess = 1/len(answers)
    best_guess_in_answers_EIG += odds_of_correct_guess

    if best_guess_EIG > best_guess_in_answers_EIG:
        return best_guess
    else:
        return best_guess_in_answers
...
def play_game(answers: list, guesses: list, target: str) -> int:
    ...
    for i in range(1,7):
        if len(answers) == 0:
            print("Solved " + target.upper() + " in " + str(i))
            return i
...
def main():
    ...
    num_guesses = play_game(answers, guesses, answer)

    print("Program got " + answer.upper() + " in " + str(num_guesses) + "!")
